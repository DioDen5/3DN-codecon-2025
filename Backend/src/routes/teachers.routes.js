                ...
                const searchRegex = new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
                    ...
                    if (university) {
                        ...
                        const universityMapping = {
                            ...
                            'ЛНУ ім. І. Франка': 'ЛНУ ім. І. Франка',
                            'Національний університет "Львівська політехніка"': 'Львівська політехніка',
                            'Львівська політехніка': 'Львівська політехніка',
                            'НУЛП': 'Львівська політехніка',
                            'Київський національний університет імені Тараса Шевченка': 'КНУ ім. Т. Шевченка',
                            'КНУ ім. Т. Шевченка': 'КНУ ім. Т. Шевченка',
                            'Національний університет "Києво-Могилянська академія"': 'НУКМА',
                            'НУКМА': 'НУКМА'
                        };

                        const normalizedUniversity = universityMapping[university] || university;

                        const possibleNames = [normalizedUniversity, university];

                        Object.keys(universityMapping).forEach(key => {
                            if (universityMapping[key] === normalizedUniversity && !possibleNames.includes(key)) {
                                possibleNames.push(key);
                            }
                        });

                        filter.university = { $in: possibleNames };
                    }
    ...
    try {
        ...
        try {
            ...
            teacherWithRating = {
                ...teacherObject,
                rating: rating,
                position: teacher.position || teacherObject.position || null
            };
