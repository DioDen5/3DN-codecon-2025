...
const FilterDropdown = forwardRef(({
    ...
    const universityOptions = (() => {
        ...
        const facultiesUnis = Array.from(new Set((facultiesData || []).map(u => u.universityName)))
            .map(name => ({ name }));

        const allUnis = universitiesData.map(u => ({
            name: u.name,
            shortName: u.shortName,
            popular: u.popular || false
        }));

        const combined = [...facultiesUnis];
        allUnis.forEach(uni => {
            if (!combined.find(u => u.name === uni.name || u.name === uni.shortName)) {
                combined.push({ name: uni.name, shortName: uni.shortName, popular: uni.popular });
            }
        });

        allUnis.forEach(uni => {
            if (uni.shortName && !combined.find(u => u.name === uni.shortName)) {
                combined.push({ name: uni.shortName, fullName: uni.name, popular: uni.popular });
            }
        });

        const count = Math.min(5, combined.length);
        const used = new Set();
        while (used.size < count) {
            const idx = Math.floor(Math.random() * combined.length);
            used.add(idx);
        }
        return combined.map((opt, idx) => used.has(idx) ? { ...opt, popular: true } : opt);
    })();
    const facultyOptions = (() => {
        ...
        const universityNameMapping = {
            ...
            'НУКМА': 'Національний університет "Києво-Могилянська академія"'
        };

        const searchName = universityNameMapping[localFilters.university] || localFilters.university;
        const uni = (facultiesData || []).find(u =>
            u.universityName === localFilters.university ||
            u.universityName === searchName
        );
